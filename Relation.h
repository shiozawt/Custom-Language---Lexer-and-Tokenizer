...
public:
	...
	// this is the whole rule
		...
		vector<string> testerA;
		vector<int> testerB;
		for (unsigned int i = 0; i < schemeVec.size(); ++i) {
			string helperHolder = schemeVec.at(i);
			if (helperHolder.at(0) == '\'') {
				
				//cout << "1" << endl;
				testerA.push_back(helperHolder);
				testerB.push_back(i);
			}
		}

		vector<vector<string>> newTupleVec;
		if (testerA.size() > 0) {
			//cout << "2" << endl;
			for (unsigned int i = 0; i < testerA.size(); ++i) {
				for (unsigned int j = 0; j < tupleVec.size(); ++j) {					
					if (tupleVec.at(j).at(testerB.at(i)) == testerA.at(i)) {
						newTupleVec.push_back(tupleVec.at(j));
						//cout << tupleVec.at(j).at(testerB.at(i)) << endl;
					}
				}		
			}
			tupleVec = newTupleVec;
		}
